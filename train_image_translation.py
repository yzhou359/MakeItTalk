...
class Image_translation_block():
    ...
    def single_test(self, jpg=None, fls=None, filename=None, prefix='', grey_only=False):
        ...
        writer = cv2.VideoWriter('out.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256, 256))#cv2.VideoWriter('out.mp4', cv2.VideoWriter_fourcc(*'mjpg'), 62.5, (256, 256))
        ...
        for i, frame in enumerate(fls):
            ...
            for i in range(g_out.shape[0]):
                #if i ==0:
                #    cv2.imwrite("/home/sathish/infant/test_image.png",g_out[0])
                frame = np.asarray(g_out[i]) * 255.0#np.concatenate((g_out[i], g_out[i], fls_in[i]), axis=1) * 255.0
                #if i==0:
                #    print(g_out.shape,ref_in.shape,fls_in.shape,frame.shape)
                ## adding alpha channel


                frame = frame.astype(np.uint8)
                #out = self.add_alpha_channel(frame)
                writer.write(frame)
